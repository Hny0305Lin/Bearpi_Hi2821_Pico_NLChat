...
#ifdef CONFIG_USBDEV_EXTEND_EP_FLUSH
...
/* Find ep by descriptor in usb controller. */
...
#define USBDEV_MAX_EPNUM          6
#define USBDEV_MAX_SG_NUM         8
...
{
#ifdef CONFIG_COMPOSITE_EXTEND_MKDEVDESC
...
/* struct usb_sg should follow these rules:
 * 1. buffer address must be cacheline align.
 * 2. alloced size must be cacheline align.
 * 3. the first num_buf buffers used in usb_sg should not be NULL.
 */

struct usb_sg {
  uint8_t *buf[USBDEV_MAX_SG_NUM];
  uint32_t len[USBDEV_MAX_SG_NUM];
  uint8_t num_buf;
};
...
struct usbdev_req_s
{
  union {
    ...
    struct usb_sg *sg_buf; /* segment buffer */
  };
  uint8_t  sg_flags;  /* 0: single buffer; 1: segment buffer */
...
struct usbdev_ep_s
{
  ...
  uint16_t mult:2;
  uint16_t maxburst:5;
  const struct usb_ss_comp_epdesc_s *comp_desc;
#endif
#endif
};
...
struct usbdev_ops_s
{
  ...
  CODE FAR struct usbdev_ep_s *(*allocep)(FAR struct usbdev_s *dev,
          uint8_t eplog, FAR struct usb_epdesc_s *desc);
#else
  CODE FAR struct usbdev_ep_s *(*allocep)(FAR struct usbdev_s *dev,
...
#ifdef CONFIG_USBDEV_ALT_UNBIND_WITH_RET
  /* Handle unbind return value. */

  CODE int
#else
  CODE void
#endif
  (*unbind)(FAR struct usbdevclass_driver_s *driver,
          FAR struct usbdev_s *dev);
