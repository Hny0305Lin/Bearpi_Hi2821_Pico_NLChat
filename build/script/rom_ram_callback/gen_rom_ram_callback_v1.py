...
input_file_list = {
    ...
    auto_redefine_macro_file_str: "auto_include_rom.h",
...
def rom_parse_region_file_list():
    ...
    with open(argv_list[region_file_str], "r") as region_file, open(
        argv_list[lds_data_file_str], "w+") as lds_data_file:
...
def classify_undef_symbols():
    with open(argv_list[rom_ram_call_func_file_str], "w+") as rom_ram_call_func_file:
        with open(argv_list[rom_ram_call_data_file_str], "w+") as rom_ram_call_data_file:
            with open(argv_list[rom_ram_call_leacy_file_str], "w+") as rom_ram_call_leacy_file:
...
def rom_gen_arm32_assemfile():
    with open(argv_list[rom_file_str], "w+") as rom_file:
        print("/*", file=rom_file)
        print(" * Copyright (c) CompanyNameMagicTag 2022-2022. All rights reserved.", file=rom_file)
        ...
        for list_line in rom_func_list:
            ...
            print(".globl %s%s" % (list_line, romcallback_suffix_str), file=rom_file)
            print(".type %s%s, %%function" % (list_line, romcallback_suffix_str), file=rom_file)
            print("%s%s:" % (list_line, romcallback_suffix_str), file=rom_file)
            print("    LDR    PC, =%s" % list_line, file=rom_file)
            print("", file=rom_file)
...
def rom_gen_riscv32_assemfile():
    with open(argv_list[rom_file_str], "w+") as rom_file:
        ...
        for list_line in rom_func_list:
            ...
            print(".type %s%s, %%function" % (list_line, romcallback_suffix_str), file=rom_file)
            print("%s%s:" % (list_line, romcallback_suffix_str), file=rom_file)
            print("    tail %s" % list_line, file=rom_file)
            print("", file=rom_file)
            veneer_rom_cnt = veneer_rom_cnt + 1
        # print(".end", file=rom_file)
        # print("// %d veneer functions" % veneer_rom_cnt, file=rom_file)
...
def rom_gen_macros_redefine_file():
    with open(argv_list[auto_redefine_macro_file_str], "w+") as auto_redefine_macro_file:
        ...
        print(" * Copyright (c) CompanyNameMagicTag 2021-2030. All rights reserved.",
              file=auto_redefine_macro_file)
        print(" * Description: Rom Callback Redefine Macros, Auto Generate by Python.", file=auto_redefine_macro_file)
        print(" * Author: Platform", file=auto_redefine_macro_file)
        # print(" * Create: %s" % time.strftime("%Y-%m-%d", time.localtime()), file=auto_redefine_macro_file)
        print(" */", file=auto_redefine_macro_file)
        print("", file=auto_redefine_macro_file)
        print("#ifndef AUTO_INCLUDE_ROM_H", file=auto_redefine_macro_file)
        print("#define AUTO_INCLUDE_ROM_H", file=auto_redefine_macro_file)
        print("", file=auto_redefine_macro_file)
        print("#ifdef _PRE_FEATURE_VENEER_ROM", file=auto_redefine_macro_file)
        macros_cnt = 0

        for list_line in rom_func_list:
            # output redefine macros to rom sections
            print("#define %s    %s%s" % (list_line, list_line, romcallback_suffix_str), file=auto_redefine_macro_file)
            macros_cnt = macros_cnt + 1
        # print("// %d macros redefine" % macros_cnt, file=auto_redefine_macro_file)
        print("#endif // _PRE_FEATURE_VENEER_ROM", file=auto_redefine_macro_file)
        print("#endif", file=auto_redefine_macro_file)
